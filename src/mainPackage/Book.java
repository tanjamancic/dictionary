...
public class Book {
		...
		try {
			...
			while (line != null) {
				...
				for (int i = 0; i < wordByWord.length; i++) {
					...
					if (wordByWord[i].isEmpty() | wordByWord[i].equals("s")) {
						continue;
					}
	...
	private ArrayList<String> sortWords(ArrayList<String> words) {     //linked list!!!
	...
	public void findOneWord (String word) {
		int numberOfRep = 0;
		for ( String w : countWords.keySet()) {
			if ( w.equals(word)) {
				numberOfRep = countWords.get(w);
				break;
			}
		}
		...
		if ( numberOfRep == 0) {
			System.out.println("Word is not used in the book.");
		} else if (numberOfRep == 1) {
			System.out.println("Word " + word + " is used once.");
		} else {
			System.out.println("Word " + word + " is repeted " + numberOfRep + " times.");
		}
	}
}
